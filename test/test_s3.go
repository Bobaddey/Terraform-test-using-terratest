...
func TestTerraformAwsS3Example(t *testing.T) {
	t.Parallel()
		...
		const name, tag = "Name", "Flugel"

		const owner, tag1 = "Owner", "InfraTeam"
		...
		expectedName := fmt.Sprintf("terratest-aws-s3-example-%s", strings.ToLower(random.UniqueId()))
			...
			// Give this EC2 Instance a unique ID for a name tag so we can distinguish it from any other EC2 Instance running
		// in your AWS account
		key1 := fmt.Sprintf("%s", name)
		value1 := fmt.Sprintf("%s", tag)

		key2 := fmt.Sprintf("%s", owner)
		value2 := fmt.Sprintf("%s", tag1)
	...
	// Verify that our bucket has was tagged properly by finding any bucket with the tags
	actualBucketTag := aws.FindS3BucketWithTag(t, awsRegion, key1, value1)
	expectedBucketTag := fmt.Sprintf("%s", bucketID)

	assert.Equal(t, expectedBucketTag, actualBucketTag)

	// Verify that our bucket has was tagged properly by finding any bucket with the owner tag
	actualBucketTag2 := aws.FindS3BucketWithTag(t, awsRegion, key2, value2)
	expectedBucketTag2 := fmt.Sprintf("%s", bucketID)

	assert.Equal(t, expectedBucketTag2, actualBucketTag2)
